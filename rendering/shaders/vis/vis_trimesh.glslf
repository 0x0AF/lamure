...
void main()
{
    ...
    if(1 == color_rendering_mode)
    {
        color = texture2D(in_mesh_color_texture, vertex_in.coord.xy).rgb;
    }

    if(2 == color_rendering_mode)
    {
        // swap y axis
        vec2 texture_coordinates = vec2(vertex_in.coord.xy);
        texture_coordinates.y = 1.0 - texture_coordinates.y;

        texture_coordinates = texture_coordinates * 0.50390625;

        color = traverse_idx_hierarchy(texture_coordinates).rgb;
    }
